...
# ---------------------------------------------------------------------------
# Patch (module-level): route top-level ``BaseException`` instances
# through ``serde.default`` BEFORE ``orjson.dumps`` sees them, so
# dataclass-based provider exceptions land in our enriched envelope
# instead of being emitted as raw dataclass fields.
#
# ``langgraph_api.serde.json_dumpb`` calls
# ``orjson.dumps(obj, default=default, option=OPT_SERIALIZE_DATACLASS)``.
# When ``obj`` is a dataclass (which every ``openrouter.errors.*``
# exception is — the SDK decorates them with ``@dataclass``), orjson
# enumerates dataclass fields directly and the ``default=`` hook is
# never invoked for the exception itself. Our
# ``_patch_serde_default_rich_exception_payload`` above therefore has
# no effect on OpenRouter's SSE error frame; the WebUI sees
# ``{"message": "...", "status_code": 401, "body": "...",
# "headers": null, "raw_response": null, "data": {...}}`` — no
# ``error``/``class``/``provider`` envelope, no way to distinguish
# quota / auth / rate-limit / model-not-found.
#
# Fix: rebind ``json_dumpb`` on every langgraph_api module that feeds
# raw exceptions to it (``stream``, ``webhook``) so ``BaseException``
# inputs go through ``serde.default`` first. serde.default returns a
# plain dict, which orjson then serializes without hitting the
# dataclass fast-path. Non-exception inputs pass through untouched.
#
# Scope: ``stream.py`` (SSE error frames) and ``webhook.py`` (webhook
# error deliveries) are the two modules that pass raw ``BaseException``
...
def _exception_shape_hook(obj: Any) -> Any:
    """Route a top-level ``BaseException`` through the patched
    ``serde.default`` so it emerges as our enriched envelope dict.
    Non-exception inputs pass through unchanged.

    Factored out as a module-level function so tests can exercise the
    shape transformation without needing ``langgraph_api.stream`` /
    ``langgraph_api.webhook`` to import cleanly (those imports trigger
    a config load that only succeeds inside a langgraph dev process).

    Failure mode: if ``serde.default`` itself raises (upstream drift,
    installer regression), we log the drift and return a minimal
    ``{error, message}`` envelope. Returning the raw exception here
    would re-introduce the dataclass bypass — the exact bug this hook
    exists to prevent — so we accept a lossy fallback (no ``provider``
    / ``class`` / ``status_code``) in exchange for a consistent
    envelope shape the WebUI can parse.
    """
    if not isinstance(obj, BaseException):
        return obj
    try:
        ...
        result = _serde_mod.default(obj)
        if isinstance(result, dict):
            return result
        # serde.default returned something other than a dict (never
        # happens today, but be defensive against future refactors).
        raise TypeError(
            f"serde.default returned {type(result).__name__}, expected dict"
        )
    except Exception as err:
        _log(
            f"exception_shape_hook_failed "
            f"cls={type(obj).__module__}.{type(obj).__qualname__} "
            f"err={type(err).__name__}: {err}"
        )
        return {"error": type(obj).__name__, "message": str(obj)}


def _patch_module_json_dumpb_dataclass_exceptions(module_name: str) -> None:
    """Rebind ``json_dumpb`` on a single langgraph_api module so raw
    ``BaseException`` args get routed through the shape hook first.
    """
    if module_name in _json_dumpb_patched_modules:
        return
    try:
        import importlib

        mod = importlib.import_module(module_name)
        _orig_json_dumpb = mod.json_dumpb

        def _patched_json_dumpb(obj: Any) -> bytes:
            return _orig_json_dumpb(_exception_shape_hook(obj))

        mod.json_dumpb = _patched_json_dumpb
        _json_dumpb_patched_modules.add(module_name)
        _log(f"json_dumpb_dataclass_bypass=applied module={module_name}")
    except Exception as install_err:
        _log(
            f"json_dumpb_dataclass_bypass=skipped module={module_name} "
            f"reason={type(install_err).__name__}: {install_err}"
        )


for _mod_name in _json_dumpb_dataclass_bypass_modules:
    _patch_module_json_dumpb_dataclass_exceptions(_mod_name)
